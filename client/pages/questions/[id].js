...
export default function answer() {
  ...
  useEffect(() => {
    (async () => {
      const { data } = await (
        await fetch(`http://54.180.175.144:8080/${idNum}?page=1&size=5`)
      ).json();
      setQuestion(data);
  ...
  const onClickAddAnswer = () => {
    ...
    fetch(
      `http://54.180.175.144:8080/answer/${localStorage.getItem(
        "user"
      )}/${idNum}`,
      {
        method: "POST",
    ...
    <div>
      ...
      <div>
        <div className="questionComentBox">
          {question
            ? question.answers.data.map((el) => {
                return <div className="answerCss">{el ? el.body : "a"}</div>;
              })
            : null}
        </div>
      </div>
      ...
      <div>
        <div className="editorContainer">
          ...
          <div>
            <div onClick={onClickAddAnswer} className="answerPostButton">
              Post Your Answer
            </div>
          </div>
        </div>
