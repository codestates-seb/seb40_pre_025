...
function isTableType(schema, type) {
    const tableNodes = [
        schema.nodes.table,
...
export function inTable(schema, selection) {
    return isTableType(schema, selection.$head.parent.type);
}
function inTableHead(schema, selection) {
...
function insertTableRowCommand(before, state, dispatch) {
    ...
    if (dispatch) {
        ...
        tableRowNode.forEach((cell) => {
            newTableCells.push(state.schema.nodes.table_cell.create(cell.attrs));
        });
        const newTableRow = state.schema.nodes.table_row.create(null, newTableCells);
...
export function insertRichTextTableCommand(state, dispatch) {
    ...
    let headerIndex = 1;
    let cellIndex = 1;
    const cell = () => state.schema.nodes.table_cell.create(null, state.schema.text(`cell ${cellIndex++}`));
    const header = () => state.schema.nodes.table_header.create(null, state.schema.text(`header ${headerIndex++}`));
    const row = (...cells) => state.schema.nodes.table_row.create(null, cells);
    const head = (row) => state.schema.nodes.table_head.create(null, row);
    const body = (...rows) => state.schema.nodes.table_body.create(null, rows);
    const table = (head, body) => state.schema.nodes.table.createChecked(null, [head, body]);
    const t = table(head(row(header(), header())), body(row(cell(), cell()), row(cell(), cell())));
